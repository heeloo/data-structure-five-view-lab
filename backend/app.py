...
def _run_demo_lldb(exe:Path,td:str,demo:dict)->dict:
    ...
    combined=f"{dbg.stdout}\n{dbg.stderr}";snapshots,program_stdout=_parse_program_output(combined);blocks=_extract_frame_blocks(combined);hits=len(re.findall(r"stop reason = breakpoint",combined,flags=re.IGNORECASE))
